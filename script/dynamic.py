...
def check_status(api_key, analysis_id,filename):
    ...
    try:
        ...
        if response.status_code == 200:
            ...
            if resp["state"]=="SUCCESS":
                ...
                with open("err_logs.txt", 'a') as f:
                    f.write(f"{filename} Error when checking status {resp['error']}")
                return True
            else:
                return False
        else:
            with open("err_logs.txt", 'a') as f:
                f.write(f"Failed to get check status. Response: {response.json()}")
            return True
    except Exception as e:
        with open("err_logs.txt", 'a') as f:
            f.write(f"Error getting analysis report: {str(e)}")
        return True
    ...
    for filename in file_list:
       ...
       # quoted_file_path = '"{}"'.format(file_path)
        ...
        if response:
            ...
            timeout=0
            while check_status(api_key,analysis_id,filename)!=True:
                timeout+=1
                time.sleep(31)
                if timeout==6:
                    with open("err_logs.txt", 'a') as f:
                        f.write(f"{filename} behaviour analysis timed out\n")
                    break

            get_analysis_report(api_key, analysis_id,filename,features_dictionary)
